...
namespace NanaArrow.Gameplay.View
{
    ...
    public sealed class ArrowViewStyle : ScriptableObject
    {
        ...
        [Header("레인 가이드 (GAME_RULES v0.7.3 §9 — 우하단 # 버튼으로 토글, 기본 꺼짐)")]
        [SerializeField, Tooltip("Arrow 머리의 행·열에 긋는 옅은 선 색 (연보라). 표 격자가 아니다")]
        private Color laneGuideColor = new Color(0.914f, 0.894f, 1f);
        [SerializeField, Range(0.005f, 0.3f), Tooltip("레인 가이드 굵기 = 셀 × 이 값")]
        private float laneGuideWidthCellRatio = 0.06f;

        [Header("빈 칸 점 (GAME_RULES v0.7.3 §9 — 토글과 무관하게 항상)")]
        [SerializeField, Tooltip("보드 안 빈 칸마다 찍는 점 (항상 표시)")]
        private bool showEmptyCellDots = true;
        [SerializeField, Tooltip("빈 칸 점 색 (연회색)")]
        private Color emptyCellDotColor = new Color(0.82f, 0.83f, 0.86f);
        [SerializeField, Range(0.02f, 0.4f), Tooltip("빈 칸 점 반지름 = 셀 × 이 값")]
        private float emptyCellDotRadiusCellRatio = 0.12f;

        [Header("정렬 순서")]
        [SerializeField, Tooltip("레인 가이드·빈 칸 점은 모든 것 뒤")] private int guideOrder = 0;
        ...
        public Color LaneGuideColor => laneGuideColor;
        public float LaneGuideWidthCellRatio => laneGuideWidthCellRatio;
        public bool ShowEmptyCellDots => showEmptyCellDots;
        public Color EmptyCellDotColor => emptyCellDotColor;
        public float EmptyCellDotRadiusCellRatio => emptyCellDotRadiusCellRatio;
        public int GuideOrder => guideOrder;
