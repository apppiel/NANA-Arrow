...
- 조립 가이드 STEP 1 에 **경고 추가**: 앵커 프리셋은 반드시 **Alt+Shift** 로 누를 것, Left/Top/Right/Bottom 이 0 인지 확인할 것, 그리고 **"플레이하면 되는데 에디터 화면이 이상하다" 면 이걸 의심할 것**
...
3. `UICanvas` 우클릭 → Create Empty → 이름 **`SafeArea`**
   ...
   - ⚠️ **Alt+Shift 를 꼭 같이 누르세요.** 그냥 클릭하면 앵커만 바뀌고 **Left/Right/Top/Bottom 이 0 이 되지 않아**, 겉보기 앵커는 stretch 인데 실제 크기는 100×100 인 상자가 됩니다. 그러면 그 안의 UI 가 전부 화면 가운데로 뭉칩니다 (2026-09-18 실제로 발생)
   - 확인법: Rect Transform 의 **Left / Top / Right / Bottom 이 전부 0** 인지 보세요. 아니면 직접 0 을 입력하면 됩니다
   - 특히 헷갈리는 점: **플레이하면 `SafeAreaAdapter` 가 런타임에 고쳐 줘서 정상으로 보입니다.** 에디터에서만 뭉쳐 보이므로 "플레이하면 되는데 화면이 이상하다" 면 이걸 의심하세요
