...
- 코드 변경만. 씬·에셋은 건드리지 않았습니다

**변경 요약 (PR)**

**`Gameplay/BoardMask`** (순수 C#, 테스트 9)
- `mask` 문자열 배열 해석. `#` = 쓰는 칸, `.` = 뺀 칸. `TryParse(rows, w, h, out mask, out error)`
- **행 순서가 뒤집혀 있습니다**: JSON 의 `mask[0]` 은 **맨 윗줄**이고 보드 셀 `(0,0)` 은 **좌하단**이라 `rows[r] ↔ y = Height-1-r` 로 대응합니다. 파일에서 사람이 모양을 눈으로 보려면 위→아래가 자연스러워서 그렇게 두고 로더가 뒤집습니다
- `Contains(x,y)` / `IncludedCount` / `ToRows()`(아스키 미리보기용)
- 크기 불일치·모르는 문자는 사람이 읽을 수 있는 메시지로 실패

**`Gameplay/Board`**
- `Board(width, height, mask = null)`, `Mask`, `IsInMask(cell)`, `UsableCellCount`
- **`IsInside` 는 그대로 사각형 기준입니다** — 지시대로 레인 판정·Fire 로직은 손대지 않았고, 마스크 밖 칸은 그냥 빈 칸으로 지나갑니다. 마스크는 **표시·검증 전용**
- 마스크 크기가 보드와 다르면 생성자에서 `ArgumentException`

**`Data`**
- `LevelData.Mask` (`string[]`, `InlineArrayConverter` 로 한 줄 출력)
- `LevelLoader.CreateBoard` 가 파싱해 Board 에 넘김. 실패 시 `FormatException` (다른 로더 오류와 동일)

**`Editor/LevelValidator` 규칙 1 확장**
- mask 크기 검사(행 수 = height, 행 길이 = width) + **모든 Arrow 셀이 mask 안**
- 오류 메시지에 `mask` 를 넣어 기획자가 원인을 바로 알 수 있게 함

**`Editor/LevelValidatorWindow`** — 기획자용 아스키 미리보기
- 레벨 카드에 `mask 27칸` 요약 + 모양을 그대로 출력 (파일을 안 열고 확인)

**`Gameplay/View/EmptyCellDots`** — **마스크 안에만** 점. `LaneGuides`·`BoardLayout` 은 지시대로 변경 없음 (바운딩 박스 = width × height)
...
. . . . . . .     ← #######
  . . . . .       ← .#####.
    . . .         ← ..###..
      A           ← ...#...
```
마스크 칸 27, 화살표 3 → **점 24개** (마스크가 없었다면 42−3=39). 레인 가이드 3선(열 1·3·5). 콘솔 에러 0.

**디렉터께 — 확인 필요**
- **`docs/LEVEL_FORMAT.md` 가 아직 v0.6 이고 `mask` 를 `int[][]` 로 "예약(v1.1)" 이라고 적고 있습니다.** WORK.md W-027 은 **문자열 배열**로 지시하셔서 그대로 구현했습니다. 문서를 v0.7 로 갱신해 주셔야 기획자가 맞게 씁니다 (타입: `string[]`, 위→아래, `#`/`.`)
- 마스크 밖 칸을 "빈 칸 취급" 하므로, **마스크 밖을 지나가는 레인도 그대로 통과**합니다. 예를 들어 하트 윗변 파인 곳(`.`) 위로 화살표가 지나갈 수 있습니다. 지시(§레인 판정 변경 없음) 그대로지만, 보기에 어색하면 알려 주세요
- 마스크가 전부 `.` 인 경우는 파싱은 통과하고 "Arrow 가 mask 밖" 오류로 걸립니다 (Arrow 가 최소 1개 필요하므로). 별도 규칙이 필요하면 추가하겠습니다

**팀장 에디터 할 일**
- 없습니다. 기획자가 `mask` 를 넣은 레벨을 올리면 **NanaArrow → Level Validator** 창에서 모양이 아스키로 보입니다
...
- 8건 전부 **플레이 모드에서 수치로 확인**했고, 이후 **팀장이 8건 모두 확인**했습니다 (3번은 실기 빌드로). 아래 표의 "확인" 은 추측이 아니라 실측값입니다
...
| 3 | 60fps | ✅ **실기 확인 완료** | 에디터: Boot 경유 시 `targetFrameRate=60`, `vSyncCount=0` / **실기 빌드에서 팀장 확인 (2026-09-18)** |
