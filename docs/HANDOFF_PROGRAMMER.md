...
### UI 함정 4개 (2026-09-18 씬 조립하며 비싸게 배운 것 — 조립 가이드에서 옮김)
- **팝업을 씬에 비활성으로 저장하면 안 되던 시절이 있었다.** `PopupBase.Awake` 가 CanvasGroup 을 잡고 뒤로가기를 구독하므로 비활성이면 Awake 가 안 돌아 `Open()` 이 NRE 였다. **W-025 에서 `Open/Close` 가 활성 상태를 직접 관리하도록 고쳐서 지금은 어느 상태로 저장돼도 동작한다.** 씬에는 비활성으로 저장하는 게 맞다 (에디터에서 안 겹쳐 보이게)
- **색이 칠해진 스프라이트에 Image.color 를 곱하지 말 것.** Image 는 스프라이트 색 × Color 다. 금색 버튼·빨간 하트에 남색(`141A33`)을 곱해 검은 덩어리가 되는 일을 **두 번** 겪었다. 그림이 있는 스프라이트면 Color 는 **흰색**이 기본
- **RectTransform 앵커 프리셋은 Alt+Shift 로 누를 것.** 그냥 누르면 앵커만 바뀌고 Left/Top/Right/Bottom 이 0 이 되지 않아 겉보기 stretch 인데 실제 100×100 상자가 된다 → 자식 UI 가 전부 화면 가운데로 뭉친다. **`SafeAreaAdapter` 가 런타임에 오프셋을 0 으로 덮어써서 플레이하면 정상으로 보이고 에디터에서만 깨진다** — "플레이는 되는데 에디터 화면이 이상하다" 면 이걸 의심
- **TMP 텍스트의 `raycastTarget` 이 버튼 클릭을 먹는다.** 버튼 위를 덮도록 stretch 된 라벨이 버튼보다 나중에 그려지면 클릭이 라벨에 먹힌다(레벨 선택 `←` 버튼이 그랬다). **글자·장식 아이콘은 `raycastTarget` 을 끈다.** 현재 세 씬 + LevelCell 프리팹의 TMP 39개는 전부 꺼 둔 상태
- **UI 는 세 층을 따로 확인해야 한다**: (1) 로직 — 메서드를 직접 호출해 동작 (2) 레이아웃 — RectTransform 값을 **에디터 상태에서** 읽기 (3) 레이캐스트 — 버튼보다 나중에 그려지면서 버튼을 덮는 `raycastTarget` 이 있는지. "메서드를 호출하니 되더라" 는 입력이 닿는다는 증거가 아니다 (W-025 5번을 이것 때문에 두 번 고쳤다)
