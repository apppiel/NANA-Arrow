# NANA-Arrow — GAME_RULES.md (v0.6 — **레퍼런스 확정에 따른 코어 전환: 화살표 = 꺾이는 경로**)
...
- 그래픽: 미니멀. 얇은 선 + 화살촉, 옅은 격자, 파스텔 배경. 색상은 `ArrowViewStyle` 로 교체 가능 (레퍼런스는 단색 클래식 + 컬러 테마 2종)
...
| 타입 | 설명 | 도입 레벨(초안) |
|---|---|---|
| Basic | 경로 1~N칸, 꺾임 자유 | 1 (직선) / 3 (첫 꺾임) |
